...
const AdminLayout = () => {
      ...
      >
        {({ isActive }) => (
          <>
            ...
            <Icon className={`w-[18px] h-[18px] flex-shrink-0 transition-colors ${isActive ? 'text-blue-600 dark:text-blue-300' : 'text-gray-400 group-hover:text-gray-600 dark:text-slate-500 dark:group-hover:text-slate-200'}`} />
  ...
  const SidebarContent = ({ isCollapsed = false }) => (
    <div className={`flex flex-col h-full bg-white dark:bg-sidebar border-r border-gray-200 dark:border-sidebar-border ${isCollapsed ? 'w-[72px]' : 'w-[260px]'} transition-all duration-300`}>
      {/* Logo */}
      <div className={`flex items-center ${isCollapsed ? 'justify-center px-3' : 'px-5'} h-16 border-b border-gray-100 dark:border-sidebar-border`}>
        <div className="flex items-center gap-3">
          ...
          {!isCollapsed && (
            <div>
              <h1 className="text-base font-extrabold text-gray-900 dark:text-sidebar-foreground tracking-tight">EventX</h1>
              <p className="text-[10px] text-gray-500 dark:text-muted-foreground font-medium uppercase tracking-widest leading-none">Studio</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={`${isCollapsed ? 'px-2' : 'px-4'} pt-4 pb-2`}>
        <Button
          className={`w-full bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold shadow-md shadow-blue-500/20 transition-all duration-300 rounded-xl h-9 ${isCollapsed ? 'px-0' : ''}`}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-6 scrollbar-thin scrollbar-thumb-gray-200 scrollbar-track-transparent">
        {/* Main */}
        <div>
          {!isCollapsed && <p className="px-3 mb-2 text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Main</p>}
        ...
        {/* Management */}
        <div>
          {!isCollapsed && <p className="px-3 mb-2 text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">Management</p>}
          <nav className="space-y-0.5">
        ...
        {/* System */}
        <div>
          {!isCollapsed && <p className="px-3 mb-2 text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">System</p>}
          <nav className="space-y-0.5">
      ...
      <div className="border-t border-gray-200 dark:border-sidebar-border p-4 space-y-3">
        ...
        <div className={`relative flex items-center ${isCollapsed ? 'justify-center' : 'gap-3 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-sidebar-accent bg-white dark:bg-sidebar border border-gray-100 dark:border-sidebar-border shadow-sm'} transition-all`}>
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0 pr-4">
              <p className="text-sm font-bold text-gray-900 dark:text-sidebar-foreground truncate">{user?.name || 'Administrator'}</p>
              <p className="text-[11px] text-gray-500 dark:text-muted-foreground truncate font-medium">{user?.email || 'admin@eventx.studio'}</p>
            </div>
          )}
        </div>
        ...
        <div className={`flex items-center ${isCollapsed ? 'flex-col gap-2' : 'justify-between gap-2'}`}>
          <button
            onClick={logout}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 hover:text-red-700 transition-all ${isCollapsed ? 'w-full px-0' : 'px-3'}`}
            title="Sign out"
          >
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen">
        {/* Header */}
        <header className="sticky top-0 z-30 bg-white/70 dark:bg-background/80 backdrop-blur-xl border-b border-gray-200/50 dark:border-border">
          <div className="flex items-center justify-between h-16 px-4 sm:px-6">
            <div className="flex items-center gap-4">
              <button
                className="md:hidden p-2 rounded-xl text-gray-500 dark:text-muted-foreground hover:bg-gray-100 dark:hover:bg-accent hover:text-gray-700 dark:hover:text-foreground transition-colors"
                onClick={() => setSidebarOpen(true)}
              >
                <Menu className="w-5 h-5" />
              </button>
              <div>
                <h2 className="text-lg font-bold text-gray-900 dark:text-foreground tracking-tight">{getPageTitle()}</h2>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={toggleTheme}
                className="p-2 text-gray-400 dark:text-muted-foreground hover:text-gray-600 dark:hover:text-foreground hover:bg-gray-100 dark:hover:bg-accent rounded-xl transition-all duration-200"
              >
              ...
              <NavLink
                ...
                className="relative p-2 text-gray-400 dark:text-muted-foreground hover:text-gray-600 dark:hover:text-foreground hover:bg-gray-100 dark:hover:bg-accent rounded-xl transition-all duration-200"
              >
              ...
              {/* Separator */}
              <div className="hidden sm:block w-px h-8 bg-gray-200 dark:bg-border mx-1" />
              ...
              <div className="relative hidden sm:block">
                ...
                >
                  ...
                  <div className="text-left">
                    <p className="text-sm font-semibold text-gray-700 dark:text-foreground leading-tight">{user?.name || 'Admin'}</p>
                    <p className="text-[11px] text-gray-400 dark:text-muted-foreground leading-tight">Administrator</p>
                  </div>
                ...
                {profileOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-popover rounded-xl shadow-xl border border-gray-100 dark:border-border py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
