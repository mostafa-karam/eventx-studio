import React, { useState } from 'react';
import { useAuth } from '../../contexts/AuthContext';
import { Button } from '../ui/button';
import { NavLink, Outlet, useNavigate } from 'react-router-dom';
import {
    Calendar, Home, Plus, Bell, User as UserIcon,
    BarChart3, Building2, Moon, Sun, LogOut, Menu, Ticket, ClipboardList, FileText
} from 'lucide-react';
import { useTheme } from '../../contexts/ThemeContext';

const OrganizerLayout = () => {
    const { user, logout } = useAuth();
    const { isDarkMode, toggleTheme } = useTheme();
    const [sidebarOpen, setSidebarOpen] = useState(false);
    const navigate = useNavigate();

    const navCls = (isActive) =>
        `w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-sm transition-colors ${isActive ? 'bg-indigo-700/60 text-white ring-1 ring-white/10' : 'text-indigo-200 hover:bg-indigo-800/50 hover:text-white'
        }`;

    const navItems = [
        { to: '/organizer/dashboard', icon: Home, label: 'Dashboard' },
        { to: '/organizer/events', icon: Calendar, label: 'My Events' },
        { to: '/organizer/halls', icon: Building2, label: 'Browse Halls' },
        { to: '/organizer/analytics', icon: BarChart3, label: 'Analytics' },
        { to: '/organizer/tickets', icon: Ticket, label: 'Ticket Management' },
        { to: '/organizer/bookings', icon: ClipboardList, label: 'My Bookings' },
        { to: '/organizer/invoices', icon: FileText, label: 'Invoice History' },
        { to: '/organizer/profile', icon: UserIcon, label: 'Profile' },
    ];

    const sidebar = (
        <div className="w-64 bg-gradient-to-b from-indigo-900 to-indigo-950 text-white flex flex-col h-full">
            <div className="p-6 border-b border-indigo-700/50">
                <div className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-indigo-500 rounded-lg flex items-center justify-center">
                        <Calendar className="w-5 h-5 text-white" />
                    </div>
                    <div>
                        <h1 className="text-xl font-bold text-white">EventX</h1>
                        <p className="text-xs text-indigo-300">Organizer</p>
                    </div>
                </div>
            </div>

            <div className="p-4">
                <Button className="w-full bg-indigo-500 hover:bg-indigo-600 text-white flex items-center space-x-2"
                    onClick={() => { navigate('/organizer/events/create'); setSidebarOpen(false); }}>
                    <Plus className="w-4 h-4" />
                    <span>Create Event</span>
                </Button>
            </div>

            <div className="flex-1 px-4 overflow-y-auto">
                <h3 className="text-sm font-medium text-indigo-300 mb-3">Navigation</h3>
                <nav className="space-y-1">
                    {navItems.map(({ to, icon: Icon, label }) => (
                        <NavLink key={to} to={to} onClick={() => setSidebarOpen(false)}
                            className={({ isActive }) => navCls(isActive)}>
                            {React.createElement(Icon, { className: 'w-4 h-4' })}
                            <span>{label}</span>
                        </NavLink>
                    ))}
                </nav>
            </div>

            <div className="p-4 border-t border-indigo-700/50">
                <div className="flex items-center space-x-3 px-3 py-2 mb-2">
                    <div className="w-7 h-7 bg-indigo-700 rounded-full flex items-center justify-center">
                        <UserIcon className="w-4 h-4 text-indigo-200" />
                    </div>
                    <div className="text-sm">
                        <p className="text-white font-medium truncate">{user?.name || 'Organizer'}</p>
                        <p className="text-indigo-300 text-xs">Event Organizer</p>
                    </div>
                </div>
                <button onClick={logout}
                    className="w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-sm text-indigo-200 hover:bg-indigo-800/50 hover:text-white transition-colors">
                    <LogOut className="w-4 h-4" />
                    <span>Logout</span>
                </button>
            </div>
        </div>
    );

    return (
        <div className="flex min-h-screen bg-background">
            <div className="hidden md:flex md:w-64 sticky top-0 self-start h-screen">{sidebar}</div>

            {sidebarOpen && (
                <div className="fixed inset-0 z-40 md:hidden">
                    <div className="absolute inset-0 bg-black/50" onClick={() => setSidebarOpen(false)} />
                    <div className="absolute left-0 top-0 h-full w-64 z-50">{sidebar}</div>
                </div>
            )}

            <div className="flex-1 min-h-0 flex flex-col">
                <header className="bg-white dark:bg-background border-b border-gray-200 dark:border-border px-4 sm:px-6 py-4 sticky top-0 z-30">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                            <button className="md:hidden p-1.5 rounded-lg text-gray-500 dark:text-muted-foreground hover:bg-gray-100 dark:hover:bg-accent"
                                onClick={() => setSidebarOpen(true)}>
                                <Menu className="w-5 h-5" />
                            </button>
                            <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-foreground">Organizer</h2>
                        </div>
                        <div className="flex items-center space-x-2 sm:space-x-4">
                            <button onClick={toggleTheme} className="p-2 text-gray-500 dark:text-muted-foreground hover:text-indigo-600 dark:hover:text-foreground hover:bg-indigo-50 dark:hover:bg-accent rounded-full transition-colors">
                                {isDarkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
                            </button>
                            <div className="hidden sm:flex items-center space-x-2 border-l border-gray-200 dark:border-border pl-4">
                                <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center">
                                    <UserIcon className="w-4 h-4 text-indigo-600" />
                                </div>
                                <div className="text-sm">
                                    <p className="font-medium text-foreground">{user?.name || 'Organizer'}</p>
                                    <p className="text-gray-500 dark:text-muted-foreground text-xs">Event Organizer</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </header>
                <main className="flex-1 overflow-y-auto">
                    <Outlet />
                </main>

            </div>
        </div>
    );
};

export default OrganizerLayout;
